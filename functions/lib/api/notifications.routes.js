...
// post /notifications/update; event updates (admins only)
router.post(
  "/update",
  requireAuth,
  requireRole("admin"),
  ...
  async (req, res, next) => {
    try {
      const created = await NotificationsService.sendUpdate(req.body);
      res.status(201).json(created);
    } catch (e) {
      next(e);
    }
...
// post /notifications/reminder; event reminders (admins only)
router.post(
  "/reminder",
  requireAuth,
  requireRole("admin"),
  ...
  async (req, res, next) => {
    try {
      const created = await NotificationsService.sendReminder(req.body);
      res.status(201).json(created);
    } catch (e) {
      next(e);
    }
